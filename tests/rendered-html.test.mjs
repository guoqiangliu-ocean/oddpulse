...
test("keeps TxLINE credentials on the server boundary", async () => {
  const [route, dashboard, envExample, gitignore] = await Promise.all([
  ...
  assert.match(route, /env as workerEnv.*cloudflare:workers/);
  assert.match(route, /process\.env\[key\]/);
  assert.match(route, /"X-Api-Token"/);
  assert.match(dashboard, /fetch\("\/api\/txline"(?:,|\))/);
  assert.doesNotMatch(dashboard, /TXLINE_API_TOKEN|X-Api-Token/);
...
test("keeps authenticated snapshots distinct from synthetic replay", async () => {
  ...
  assert.match(route, /participant1IsHome/);
  assert.match(route, /mode: "authenticated-snapshot"/);
  assert.match(route, /fetchedAt: Date\.now\(\)/);
  assert.match(route, /timestamp: raw\.Ts \?\? raw\.ts \?\? null/);
  assert.match(route, /competition: raw\.Competition \?\? raw\.competition \?\? "unknown"/);
  assert.match(route, /Number\.isSafeInteger\(fixture\.fixtureId\)/);
